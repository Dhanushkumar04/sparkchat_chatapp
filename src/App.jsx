import './App.css'
import 'bootstrap/dist/css/bootstrap.min.css'
import Chat from './components/Chat'
import { useState, useEffect } from 'react'
import { signInWithPopup, signOut, onAuthStateChanged } from 'firebase/auth'
import { auth, provider } from './config/firebase'
import logoImg from './assets/logo.jpeg'

function App() {
  const [user, setUser] = useState(null)
  const [loading, setLoading] = useState(true)

  // Listen to Auth state change (keeps user logged in on page refresh!)
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      setUser(currentUser)
      setLoading(false)
    })
    return () => unsubscribe()
  }, [])

  const handleSignIn = () => {
    signInWithPopup(auth, provider)
      .then((result) => {
        setUser(result.user)
      })
      .catch((error) => console.error("Sign in error:", error))
  }

  const handleSignOut = () => {
    signOut(auth)
      .then(() => {
        setUser(null)
      })
      .catch((error) => console.error("Sign out error:", error))
  }

  if (loading) {
    return (
      <div className="App d-flex align-items-center justify-content-center vh-100">
        <div className="spinner-border text-indigo" role="status" style={{ color: '#6366f1' }}>
          <span className="visually-hidden">Loading...</span>
        </div>
      </div>
    )
  }

  return (
    <div className="App">
      {/* Dynamic Floating Background Orbs for 3D Depth */}
      <div className="bg-orb orb-indigo"></div>
      <div className="bg-orb orb-violet"></div>
      <div className="bg-orb orb-cyan"></div>

      {user ? (
        <Chat user={user} onSignOut={handleSignOut} />
      ) : (
        <div className="login-card">
          <div className="login-logo-container">
            <img 
              src={logoImg}
              alt="SparkChat Logo" 
              className="login-logo"
            />
          </div>
          <h2 className="login-title">Welcome to SparkChat</h2>
          <p className="login-subtitle">
            Connect and chat in realtime with your team. Sign in with Google to start instantly 🚀
          </p>
          <button 
            className="btn-google"
            onClick={handleSignIn}
          >
            {/* Google Colorful SVG Logo */}
            <svg className="google-icon" viewBox="0 0 24 24">
              <path
                fill="#4285F4"
                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
              />
              <path
                fill="#34A853"
                d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
              />
              <path
                fill="#FBBC05"
                d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z"
              />
              <path
                fill="#EA4335"
                d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z"
              />
            </svg>
            Continue with Google
          </button>
        </div>
      )}
    </div>
  )
}

export default App
